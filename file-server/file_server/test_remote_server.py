...
class TestRemoteServer(TestServer):
    ...
    def test_create_file(self):
        ...
        self.assertEqual(file_1_metadata.get('file-store-usage'), 0)
        ...
        self.assertEqual(file_2_metadata.get('file-size'), 0)
        self.assertEqual(file_2_metadata.get('file-store-usage'), 1000)
        self.assertEqual(file_1_metadata.get('file-chunks'), 0)
    ...
    def test_commit_file(self):
        ...
        self.assertEqual(file_1_metadata.get('file-store-usage'), 1000)
        self.assertEqual(file_1_metadata.get('file-chunks'), 1)
        self.assertEqual(file_1_metadata.get('is-committed'), False)
        self.assertEqual(file_1_metadata.get('created-epoch-no'), None)
        ...
        self.assertEqual(file_1_metadata.get('created-epoch-no'), 1)
        ...
        self.assertTrue(file_2_metadata is not None)
        self.assertEqual(file_2_metadata.get('remote-file-id'), file_2_id)
        self.assertEqual(file_2_metadata.get('file-store-usage'), 1500)
        self.assertEqual(file_2_metadata.get('file-size'), 1500)
        self.assertEqual(file_2_metadata.get('file-chunks'), 2)
    ...
    def test_write_remote_file(self):
        ...
        self.assertEqual(file_1_metadata.get('file-store-usage'), 1500)
        self.assertEqual(file_1_metadata.get('file-chunks'), 2)
        self.assertEqual(file_1_metadata.get('is-committed'), True)
        r = self.send_request(URL.format('/1/file?size=1000'), method=requests.post, headers=req_headers)
        self.assertEqual(r.status_code, HTTPStatus.OK)
        file_2_id = r.headers.get('x-privastore-remote-file-id')
        chunk_4 = random.randbytes(50)
        r = self.send_request(URL.format('/1/file/{}?chunk=1'.format(file_2_id)), data=chunk_4, method=requests.put, headers=req_headers)
        self.assertEqual(r.status_code, HTTPStatus.OK)
        r = self.send_request(URL.format('/1/file/{}/commit'.format(file_2_id)), method=requests.put, headers=req_headers)
        self.assertEqual(r.status_code, HTTPStatus.OK)
        file_2_metadata = self.send_request(URL.format('/1/file/{}/metadata'.format(file_2_id)), headers=req_headers)
        self.assertTrue(file_2_metadata is not None)
        self.assertEqual(file_2_metadata.get('remote-file-id'), file_2_id)
        self.assertEqual(file_2_metadata.get('file-size'), 50)
        self.assertEqual(file_2_metadata.get('file-store-usage'), 50)
        self.assertEqual(file_2_metadata.get('file-chunks'), 1)
        self.assertEqual(file_2_metadata.get('is-committed'), True)
    ...
    def test_remove_file(self):
        ...
        self.assertTrue(file_1_metadata is not None)
        self.assertEqual(file_1_metadata.get('is-committed'), False)
        file_2_metadata = self.send_request(URL.format('/1/file/{}/metadata'.format(file_2_id)), method=requests.get, headers=req_headers)
        self.assertTrue(file_2_metadata is not None)
        self.assertEqual(file_2_metadata.get('is-committed'), True)
        r = self.send_request(URL.format('/1/file/{}'.format(file_1_id)), method=requests.delete, headers=req_headers)
        self.assertEqual(r.status_code, HTTPStatus.OK)
        r = self.send_request(URL.format('/1/file/{}/metadata'.format(file_1_id)), method=requests.get, headers=req_headers)
        self.assertTrue(r.json()['error'], 'FILE_NOT_FOUND')
        r = self.send_request(URL.format('/1/epoch/1'), method=requests.put, headers=req_headers)
        self.assertEqual(r.status_code, HTTPStatus.OK)
        r = self.send_request(URL.format('/1/file?size=1000'), method=requests.post, headers=req_headers)
        self.assertEqual(r.status_code, HTTPStatus.OK)
        file_3_id = r.headers.get('x-privastore-remote-file-id')
        self.assertTrue(file_3_id is not None)
        self.assertNotEqual(file_3_id, file_1_id)
        self.assertNotEqual(file_3_id, file_2_id)
        r = self.send_request(URL.format('/1/file/{}?chunk=1'.format(file_3_id)), data=chunk_1, method=requests.put, headers=req_headers)
        self.assertEqual(r.status_code, HTTPStatus.OK)
        req_headers['x-privastore-epoch-no'] = '2'
        r = self.send_request(URL.format('/1/file/{}/commit'.format(file_3_id)), method=requests.put, headers=req_headers)
        self.assertEqual(r.status_code, HTTPStatus.OK)
        file_2_metadata = self.send_request(URL.format('/1/file/{}/metadata'.format(file_2_id)), method=requests.get, headers=req_headers)
        self.assertTrue(file_2_metadata is not None)
        self.assertEqual(file_2_metadata.get('created-epoch-no'), 1)
        self.assertEqual(file_2_metadata.get('is-committed'), True)
        file_3_metadata = self.send_request(URL.format('/1/file/{}/metadata'.format(file_3_id)), method=requests.get, headers=req_headers)
        self.assertTrue(file_3_metadata is not None)
        self.assertEqual(file_3_metadata.get('created-epoch-no'), 2)
        self.assertEqual(file_3_metadata.get('is-committed'), True)
        r = self.send_request(URL.format('/1/file/{}'.format(file_2_id)), method=requests.delete, headers=req_headers)
        self.assertEqual(r.status_code, HTTPStatus.OK)
        r = self.send_request(URL.format('/1/file/{}'.format(file_3_id)), method=requests.delete, headers=req_headers)
        self.assertEqual(r.status_code, HTTPStatus.OK)
        r = self.send_request(URL.format('/1/file/{}/metadata'.format(file_2_id)), method=requests.get, headers=req_headers)
        self.assertTrue(r.json()['error'], 'FILE_NOT_FOUND')
        r = self.send_request(URL.format('/1/file/{}/metadata'.format(file_3_id)), method=requests.get, headers=req_headers)
        self.assertTrue(r.json()['error'], 'FILE_NOT_FOUND')
        r = self.send_request(URL.format('/1/epoch/2'), method=requests.put, headers=req_headers)
        self.assertEqual(r.status_code, HTTPStatus.OK)
